...
<html lang="en">
  ...
  <body style="background-color: #F5F5F5;">
    <div class="topnav">
      ...
      <a class="active" href="event.php">Placements/Events</a>
    ...
    <div class="container shadow">
        ...
        <?php
            ...
            $sql = "SELECT user_type FROM users WHERE user_id = $val";
            $query = mysqli_query($conn, $sql);
            //echo $sql;
            $rowData = mysqli_fetch_array($query);
            //echo $rowData['user_type'];
            if($rowData['user_type'] == 'Faculty'){
                //echo '$query';
      ...
      <div data-spy="scroll"  data-offset="0" class="scrollspy-example container">
        ...
        <?php
              //$val = $_SESSION['user_id'];
              $sql = "SELECT event_id, event_title, event_content, event_time, event_img_url, event_org_url, event_by FROM events ORDER BY event_time DESC";
              $query = mysqli_query($conn, $sql);
              //echo $sql;
              //$rowData = mysqli_fetch_array($query);
              if (mysqli_num_rows($query) > 0) {
                while($rowData = mysqli_fetch_array($query)){
                    $val = $rowData["event_by"];
                    ...
                    echo '<span class="element2">'.$rowData["event_time"].'</span>';
                    echo '</div>';
                    
                    echo '<hr class="new1">';
                    echo '<span align="left">Title :&nbsp;'.$rowData["event_title"].'</span>';
                    echo '<span align="left">Content :&nbsp;'.$rowData["event_content"].'</span>';
                    if ($rowData["event_img_url"]!=""){
                      $imag = pathinfo($rowData["event_img_url"], PATHINFO_EXTENSION);
                      if ($imag == "png" || $imag == "jpg"||$imag == "jpeg"){
                        echo '<center><img src="uploads/events/'.$rowData["event_img_url"].'"height="500px" width="500px"></center>';
                      }
                      //$imag = pathinfo($rowData["post_img_url"], PATHINFO_EXTENSION);
                      //echo $imag;
                      else{
                        echo '<a href="uploads/events/'.$rowData["event_img_url"].'" target="_blank">'.$rowData["event_org_url"].'</a>';
                      }
                    }
        ...
        /*if ($img_size > 12500000) {
          ...
          header("Location: home.php?error=$em");
            ...
            if(TRUE){  
                ...
                $img_upload_path = 'uploads/events/'.$new_img_name;
                ...
                header("Location: event.php?error=$em");
            }
        }
      //}
    // Apply Insertion Query
    $sql = "INSERT INTO events (event_title, event_content, event_time, event_img_url, event_org_url, event_by)
